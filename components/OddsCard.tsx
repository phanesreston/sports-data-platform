...
function OddsButton({ label, odds, bookmaker }: OddsButtonProps) {
  return (
    <div className="flex flex-1 flex-col items-center">
      ...
      <button className="odds-btn group w-full rounded-xl border border-bg-border bg-bg-surface px-3 py-2.5 text-center transition-all hover:border-accent-green/50 hover:bg-accent-green/5">
        <span className="block text-base font-bold text-white group-hover:text-accent-green">
          {odds.toFixed(2)}
        </span>
        <span className="block truncate text-xs text-slate-600 group-hover:text-slate-400">
          {bookmaker}
        </span>
      </button>
    </div>
